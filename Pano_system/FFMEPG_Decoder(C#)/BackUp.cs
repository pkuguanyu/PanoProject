...
{
    public unsafe class VRpalyer_Backup
    {
        ...
        static void Main_2()
        {
            ...
            //s.SDL_Init(960, 960);
            string video = "I:/simplest_ffmpeg_player-master/simplest_ffmpeg_player-master/simplest_ffmpeg_player_su/0.mp4";
                ...
                if (videoindex > -1)
                {
                    ...
                    while (ffmpeg.av_read_frame(ofmt_ctx, packet) >= 0)
                    {
                        if (flag == 0)
                        {
                            ...
                            {
                                m.WaitOne();
                                if (Resource[tilenumber] == 1)
                                {
                                    Resource[tilenumber] = Resource[tilenumber] - 1;
                                    m.ReleaseMutex();
                                    break;
                                }
                                m.ReleaseMutex();
            ...
            while (true)  //不停的向前进行decode,每一次循环产生一帧
            {
                ...
                if (T % (changeFrame - 1) == 1)
                {
                    ...
                    for (i = 0; i < tileNumber; i++)
                    {
                        isTrue[i] = false;
                        Resource[i] = 0;
                        string temp1 = "0.mp4";// i.ToString()+".mp4";
                        int tn = i;
