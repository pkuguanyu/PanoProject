    ...
    try
        ...
        for x=8:mm
            for y=9:nn
                [x y]
                flag=0;
                XXXX=struct2cell(load(['I:\视频评价调研\Potential improment\第六版Potential improvement\PMSE\R\cd\',set,'\',num2str(videoid),'\',num2str(frame),'_',num2str(x),'_',num2str(y),'\X.mat']));
                ...
                for qp=42:-1:22
                    ...
                    fitTypeMseToBitrate_A=fittype('a.*x.^b +c','independent','x','coefficients',{'a','b','c'});
                    opt_A=fitoptions(fitTypeMseToBitrate_A);
                    opt_A.StartPoint=[max(A_reason(:)),-3,min(A_reason(:))];
                    ...
                    fitTypeMseToBitrate_C=fittype('a.*x.^b +c','independent','x','coefficients',{'a','b','c'});
                    opt_C=fitoptions(fitTypeMseToBitrate_C);
                    opt_C.StartPoint=[max(C_reason(:)),-3,min(C_reason(:))];
...
%                     end
                       ...
                       for i=1:step-1
                            ...
                            YY=tempY(i:step:end)-(max(tempY(i:step:end))+min(tempY(i:step:end)))/2;
                            y2=@(x,a,b) a*sin(x+b);
                            data_p=[data_p,y2(AA,result_A(tempX(i)),B)+result_C(tempX(i))];
                            data_r=[data_r,YY+(max(tempY(i:step:end))+min(tempY(i:step:end)))/2];
                       end
        ...
        mkdir(['I:\视频评价调研\Potential improment\Fit_error\',num2str(ss),'\',num2str(videoid)]);
        save(['I:\视频评价调研\Potential improment\Fit_error\',num2str(ss),'\',num2str(videoid),'\',num2str(frame),'_p.mat'],'data_p');
        save(['I:\视频评价调研\Potential improment\Fit_error\',num2str(ss),'\',num2str(videoid),'\',num2str(frame),'_r.mat'],'data_r');
     catch
         [frame]
          mkdir(['I:\视频评价调研\Potential improment\Fit_error\',num2str(ss),'\',num2str(videoid)]);
          save(['I:\视频评价调研\Potential improment\Fit_error\',num2str(ss),'\',num2str(videoid),'\',num2str(frame),'_p.mat'],'data_p');
