...
outpupath="D:/2018.3.27/User/Range/ComparasionLeaveRange_"+str(gap)+"s/" #记录高离开率的动作信息
outpupath2="D:/2018.3.27/User/Range/ComparasionStayRange_"+str(gap)+"s/" #记录正常离开率的动作信息
    ...
    for i in range(0,1):
        ...
        for stay in StayUser:
            ...
            for j in range(0,len(line2)):
                sum_stay[j]=sum_stay[j]+float(line2[j])
                total[j]=total[j]+1
        ...
        for leave in LeaveUser:
            ...
            line2 = file2.readlines()
            for j in range(0, len(line2)):
                sum_leave[j] = sum_leave[j] + float(line2[j])
                total[j] = total[j] + 1
