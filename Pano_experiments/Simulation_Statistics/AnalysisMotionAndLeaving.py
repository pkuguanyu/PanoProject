...
path2="C:/Aiqiyi/DirectionResult"+str(gap)+"/"
outpupath="D:/2018.3.27/Direction/ComparasionHigh_"+str(gap)+"s/" #记录高离开率的动作信息
outpupath2="D:/2018.3.27/Direction/ComparasionNormal_"+str(gap)+"s/" #记录正常离开率的动作信息
...
for filename in filename_list:  #这个是统计过离开率的视频，要找原来的初始的视频
    ...
    for i in range(0,end2):
        ...
        for j in range(locationStart,locationEnd):
                ...
                file2 = open(path2 + filename_raw, 'r')
                lines2 = file2.readlines()
            #这一部分计算
                for line in lines2:
                    templine=line.split(" ")
                    timesatampStart=float(templine[0])
                    timesatampEnd=float(templine[1])
                    data=float(templine[2])
                    if abs(startTime-timesatampStart)<=1 or abs(endTime-timesatampEnd)<=1:
        ...
        print t
                        # i对应i号时间戳的平均运动方向改变次数
        if sum_high_normal[i] != -1:
                sum_high_normal[i] = sum_high_normal[i] / (gap * t)
